...
use crate::memory::flat_memory::{flat_mem, dram_entry};
...
impl CPU {
    fn eval_MEM(agu_mem_rf: &AGU_MEM_rf, fmem: &flat_mem) -> (MEM_WB_RF, signal_req, Vec<arch_action>) {
                ...
                DMAop::READ_VEC => {
                    if let Some(paddr) = agu_mem_rf.get_phys_addr() {
                        (
                            ...
                            signal_req::new(signal_reason::MEM_block, CPU_stages::MEM, 
                                Some(HashSet::<CPU_stages>::from([
                                             CPU_stages::IF, CPU_stages::ID,
                                            CPU_stages::EX, CPU_stages::AGU, CPU_stages::MEM]))),
                            [arch_action::DoNothing].to_vec()
                        )
                    } else {
                        (
                ...
                DMAop::WRITE_VEC { data_lit } => {
                    if let Some(paddr) = agu_mem_rf.get_phys_addr() {
                        (
                            ...
                            signal_req::new(signal_reason::no_reason, CPU_stages::MEM, None),
                            [arch_action::WriteMEM_DATA{addr: paddr, content: data_lit}].to_vec()
                        )
                    } else {
                        (
                            MEM_WB_RF{
                ...
                DMAop::WRITE_FPTR { fptr_data_lit } => {
                    if let Some(paddr) = agu_mem_rf.get_phys_addr() {
                        (
                            ...
                            signal_req::new(signal_reason::no_reason, CPU_stages::MEM, None),
                            [arch_action::WriteMEM_FPTR{addr: paddr, content: fptr_data_lit}].to_vec()
                        )
                    } else {
                        (
                            MEM_WB_RF{
